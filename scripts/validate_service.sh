...
if [ docker container inspect -f '{{.State.Running}}' "$(docker ps --filter name=name=pauloeduardods-front-end-1 -nq)" == "true" ]; then
  ...
  docker rmi -f pauloeduardods-front-end:latest
  docker image tag pauloeduardods-front-end_old:latest pauloeduardods-front-end:latest
  docker rmi -f pauloeduardods-front-end_old:latest
